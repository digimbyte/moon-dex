...
namespace sc.splines.spawner.runtime
{
    public static class SplineFunctions
    {
        ...
        [BurstCompile]
        public static bool IsInsideSpline(this NativeSpline spline, NativeArray<SplinePoint> splinePoints, float splineLength, float3 position, float searchInterval, float margin, out float3 nearestPosition)
        {
            float paddingSquared = margin * margin;
            int sampleCount = (int)math.floor(splineLength / searchInterval);

            SplineCache.Sample(ref splinePoints, 0, out SplinePoint splinePoint);
            
            nearestPosition = splinePoint.position;
            ...
            for (int i = 1; i <= sampleCount; i++)
            {
                ...
                SplineCache.Sample(ref splinePoints, t, out splinePoint);
                float3 splinePosition = splinePoint.position;
                
                //Slow realtime sampling
                //float3 splinePosition = spline.EvaluatePosition(t);
                
                //Check for distance from spline
                float distSq = math.distancesq(splinePosition, position);
                ...
                if ((previousPosition.x <= position.x && splinePosition.x > position.x) || 
                    (splinePosition.x <= position.x && previousPosition.x > position.x))
                {
        ...
        public static bool FindNearestPointWithinSpline(this NativeSpline spline, NativeArray<SplinePoint> splinePoints, float splineLength, float3 start, float3 end, float3 direction, float sampleIntervalDistance, float margin, out float3 nearestPosition)
        {
            ...
            for (int j = 0; j < sampleCount; j++)
            {
                ...
                if (spline.IsInsideSpline(splinePoints, splineLength, samplePos, sampleIntervalDistance, margin, out var nearest))
                {
