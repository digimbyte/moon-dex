...
namespace sc.splines.spawner.runtime
{
    ...
    public struct SpawnOnGrid : IJob
    {
        ...
        public SpawnOnGrid(NativeSpline targetSpline, NativeArray<SplinePoint> splinePoints, float4x4 localToWorld, DistributionSettings distributionSettings, NativeList<PrefabData> prefabData,
        ...
        void CreateLanes(float3 axis, float3 direction, float width, float length, int count, float cellSize)
        {
            ...
            for (int i = startIndex; i <= endIndex; i++)
            {
                ...
                {
                    startInside = spline.IsInsideSpline(splinePoints, splineLength, start, searchIntervalDistance, areaMargin, out var _);

                    //Point falls outside of spline, start tracing inwards to find the first point that is
                    if (startInside == false)
                    {
                        if (spline.FindNearestPointWithinSpline(splinePoints, splineLength, start, end, direction, searchIntervalDistance, areaMargin, out float3 nearestStart))
                        {
                            start = nearestStart;
                            startInside = true;
                        }
                    ...
                    endInside = spline.IsInsideSpline(splinePoints, splineLength, end, searchIntervalDistance, areaMargin, out var _);

                    if (endInside == false)
                    {
                        if (spline.FindNearestPointWithinSpline(splinePoints, splineLength, end, start, -direction, searchIntervalDistance, areaMargin, out float3 nearestEnd))
                        {
                            end = nearestEnd;
                            endInside = true;
                        }
                ...
                for (int j = 0; j < pointCount; j++)
                {
                    ...
                    if (concaveSupport)
                    {
                        bool isInside = spline.IsInsideSpline(splinePoints, splineLength, spawnPos, searchIntervalDistance, areaMargin, out nearestPosition);
