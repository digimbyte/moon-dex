...
namespace sc.splines.spawner.runtime
{
    ...
    public class SplineSpawnerMask : MonoBehaviour
    {
        ...
        List<int2> positionIndices = new List<int2>();
        ...
        public void RenderSDFIfNeeded()
        {
            ...
            if (positionIndices == null)
                positionIndices = new List<int2>(splineCount);
            else
            ...
            for (int i = 0; i < splineCount; i++)
            {
                Spline spline = splineContainer[i];
                float splineLength = spline.CalculateLength(splineContainer.transform.localToWorldMatrix);
                
                //Invalid spline
                if(splineLength < 1 || spline.Count == 0) continue;
                ...
                int2 indexRange = new int2(lastIndex, (lastIndex) + sampleCount);
                positionIndices.Add(indexRange);
                lastIndex = indexRange.y;
            }

            //Recount splines, so invalid (0 length) splines are not really included
            splineCount = positionIndices.Count;
        ...
        /// <summary>
        /// Checks all active Spline Spawner component for intersection between their splines and the ones used for this mask, and the configured layer mask
        /// </summary>
        /// <returns></returns>
        public List<SplineSpawner> GetSpawnersAffectedBy()
        {
            SplineSpawner[] spawners = FindObjectsByType<SplineSpawner>(FindObjectsInactive.Exclude);
