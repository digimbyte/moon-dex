...
namespace sc.splines.spawner.editor
{
    ...
    class SplineSpawnerMaskInspector : Editor
    {
        ...
        private List<SplineSpawner> affectedSpawners;
        ...
        private void OnEnable()
        {
            ...
            affectedSpawners = script.GetSpawnersAffectedBy();
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField($"Affected spawners ({affectedSpawners.Count})", EditorStyles.boldLabel);
            using (new EditorGUILayout.VerticalScope(EditorStyles.textArea))
            {
                foreach (var spawner in affectedSpawners)
                {
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        EditorGUILayout.LabelField($"• {spawner.name}", EditorStyles.miniLabel);
                        if (GUILayout.Button("Select", EditorStyles.miniButton, GUILayout.MaxWidth(80f)))
                        {
